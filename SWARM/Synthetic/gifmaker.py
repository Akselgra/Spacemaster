...
maxshift = 9.99
shifts = np.linspace(0, maxshift, frames)



for shift in shifts:
    times2 = np.linspace(shift, t + shift, n)
    data2 = (np.sin(times2*f2*2*np.pi) + np.sin(times2*f0*2*np.pi))/2
    ...
    plt.title("Signals, Shifted %2.2f seconds" % (shift))
    plt.axis([0, t, -1.1, 1.1])
    plt.savefig("signal_gif/signal_shift=000%2.2f.png" % (shift))
    plt.close()

    plt.plot(freqs, np.abs(cross_spec))
    plt.xlabel("Frequency [Hz]")
    plt.ylabel("Power")
    plt.title("CSD, shifted %2.2f seconds" % (shift))
    plt.axis([-fs/1.9, fs/1.9, np.min(np.abs(cross_spec)), np.max(np.abs(cross_spec))])
    plt.savefig("CSD_gif/CSD_shift=000%2.2f.png" % (shift))
    plt.close()
    ...
    plt.xlabel("Frequency [Hz]")
    plt.ylabel("Phase")
    plt.title("Phase, shifted %2.2f seconds" % (shift))
    plt.savefig("phase_gif/phase_shift=000%2.2f.png" % (shift))
    plt.close()
